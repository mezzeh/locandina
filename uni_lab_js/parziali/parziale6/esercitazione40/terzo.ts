...
function groupByItems<T extends {key:string}>(item: T[]): {[key:string]:T[]}
{


  let diz :  {[key:string]:T[]} = {}
  for (let el of item)
  {
...
function groupUniqueByName<T extends {key:string,name:string}>(item: T[]): {[key:string]:T[]}
{
...
function countGroups<T extends {key:string}>(groups:{[key: string]: T[]}): number
{
  let cont = 0;
  let set = new Set()
