...
class Trie {
    ...
    insert(key: string): void {
        ...
        for (let c of key) {
            ...
            node = node.children.get(c)! // L'operatore "!" asserisce che "node.children.get(c)" (di tipo TrieNode | undefined) non è undefined, permettendone l'assegnazione a "node" (di tipo TrieNode)
        }
        if (!node.isTerminal) {
    ...
    *prefixSearch(prefix: string): Generator<string> {
        ...
        for (let c of prefix) {
            if (!node.children.has(c))
                return // se neanche ha il prefisso esci.
            node = node.children.get(c)! // scorre tutto il prefisso. ora il nodo attuale è quello che ha l'ultimo carattere del prefisso
        }
        yield* this.collectAll(node, prefix) // delega la generazione di ogni parola avente questo prefisso? quindi devono ricontrollare che abbia il prefisso?
    }

    private *collectAll(node: TrieNode, prefix: string): Generator<string> {
        if (node.isTerminal) // la parola qua diventa il prefisso., nel momento in cui non ci sono altre lettere davanti ritorna la fine.
            yield prefix // se passato è gia terminato significa che la parola è il preisso.
      for (let [char, child] of node.children) { // prende char ,child, ma la strittura di node.children è una mappa? no no è corretto
          // prende solo quelli validi. : a: NodoA->  // controlla tutti quelli con ab _> aba -> abc...
        yield* this.collectAll(child, prefix + char) // delega il controllo del prossimo ad un altro-> aggiungendo il carattere successivo a quello che eggettivamente c'e
        // -> abc
        }
    }
