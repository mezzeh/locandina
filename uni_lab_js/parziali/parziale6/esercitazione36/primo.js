...
class Movimento {
    ...
    constructor(tipo, euro, motivo, data) {
        if (!Movimento.tipi.has(tipo))
            throw new TypeError("movimento sconosociuto");
        if (motivo == "")
            throw new TypeError("motivo non valido");
        this.#tipo = tipo;
        this.#euro = euro;
        this.#motivo = motivo;
        this.#data = data;
...
class Parcometro {
    ...
    storico(k) {
        let mvs = (this.#movimenti.reverse()).splice(0, k);
        //let MvJson = mvs.forEach((mv) => JSON.stringify(mv)) // foreach è mmutile
        let Mvs = mvs.map((x) => JSON.stringify(x));
        return Mvs;
    }
    static storicoGlobale() {
        let Mvs = Parcometro.registro.map((x) => ({ movimento: JSON.stringify(x.movimento), istanza: x.istanza })); // devo
        return Mvs;
    }
}
let str = "d";
let parcometro = new Parcometro(str, 5);
