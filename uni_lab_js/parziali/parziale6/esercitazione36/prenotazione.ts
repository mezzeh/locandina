class Passeggero
{
  #nome: string; #categoria: CategoriaPasseggero;
...
type statoPrenotazine = "aperta" | "chiusa";
type CategoriaPasseggero = "adulto"| "minore";
class Prenotazione
{
  #codice: string;
  #posti: number;
  #stato: statoPrenotazine= "aperta";
...
class PrenotazionePremium extends Prenotazione
{
  ...
  elencoPremium()
  {
    ...
    let base :Passeggero[]= super.elenco()
    let out :premiumP[] = []
    for(let el of base)
    {
      out.push({
  ...
  static chiudiPiene(xs:unknown[]):Prenotazione[]
  {
    let arr : Prenotazione[] = [];
...
function assertThrows(fn: () => void, msg: string) {
  try { fn(); console.log(`✗ FALLITO (nessuna eccezione): ${msg}`); }
  catch { console.log(`✓ ${msg}`); }
}

// ── Passeggero ──────────────────────────────────────────────
assertThrows(() => new Passeggero("", "adulto"),           "nome vuoto → TypeError");
assertThrows(() => new Passeggero("Luca", "bambino" as any),"categoria non valida → TypeError");
